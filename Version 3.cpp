...
int main(void)
{
	...
	while (optionInput != 3) {
		...
		if (optionInput == 1) {
			...
			DAQmxErrChk(DAQmxCfgSampClkTiming(taskHandle, "", inputFrequency * 10, DAQmx_Val_Rising, DAQmx_Val_FiniteSamps, inputFrequency * inputRuntime * 100)); //DAQmx_Val_ContSamps
			DAQmxErrChk(DAQmxCfgDigEdgeStartTrig(taskHandle, "/Dev1/PFI0", DAQmx_Val_Rising));
